...
test('URI encodes "href" property', (t) => {
  ...
  t.is(getPropertyValue(node, 'href'), 'http://foo.tdl/foo%20bar');
});
...
test('URI encodes "src" property', (t) => {
  ...
  t.is(getPropertyValue(node, 'src'), 'http://foo.tdl/foo%20bar');
});
...
test('returns textContent property value', (t) => {
  ...
  t.is(getPropertyValue(node, 'textContent'), 'foo');
});
...
test('returns outerHTML property value', (t) => {
  ...
  t.is(getPropertyValue(node, 'outerHTML'), '<div>foo</div>');
});
...
test.skip('returns outerHTML property value (table)', (t) => {
  ...
  t.is(getPropertyValue(nodes[0], 'outerHTML'), '<td>foo</td>');
});
...
test('returns outerHTML property value (tag within a tag)', (t) => {
  ...
  t.is(getPropertyValue(nodes[0], 'outerHTML'), '<script>"<br>"</script>');
});
